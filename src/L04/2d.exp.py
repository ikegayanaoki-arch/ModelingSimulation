...
for i in range(1,N+1):
  ce[i] = dl/(0.5*(dx[i]+dx[i+1]))
  cw[i] = dl/(0.5*(dx[i]+dx[i-1]))
  cn[i] = dl/(0.5*(dy[i]+dy[i+1]))
  cs[i] = dl/(0.5*(dy[i]+dy[i-1]))
#ce[:]=1.0
#cw[:]=1.0
#cn[:]=1.0
#cs[:]=1.0
...
while flg == 1:
   ...
   for j in range(1,N+1): 
       for i in range(1,N+1): 
           ...
           T[j,i] = (ce[i]*Tp[j,i+1] + cw[i]*Tp[j,i-1]\
                    +cn[j]*Tp[j+1,i] + cs[j]*Tp[j-1,i]\
                    -q[j,i]*dx[i]*dy[i]/lm)/co[j,i] 
