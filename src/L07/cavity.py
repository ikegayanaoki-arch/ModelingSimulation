...
import sys
## Exercise #######################################################
#  i) Write equation for u and v
#  ii) Calculate the cavity flow at Re=1, 10, 1000, and compare the results. 
###################################################################
...
#Re=float(args[1])
Re   = 1000.0    #Reynolds number = nu ut/L
## definition
...
ax.set_ylim(0,1)
ax.set_xlim(0,1)
levelsp=np.linspace(-0.4,0.4,41) ## for p
levelsu=np.linspace(0,1,41)      ## for u, v
...
ani.save('cavity.gif', writer="imagemagick")
plt.show()
